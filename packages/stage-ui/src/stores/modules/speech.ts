...
export const useSpeechStore = defineStore('speech', () => {
  ...
  function getMetadataForProvider(provider: string) {
    if (!provider)
      return undefined

    return providersStore.providerMetadata[provider]
  }
  ...
  const supportsModelListing = computed(() => {
    return getMetadataForProvider(activeSpeechProvider.value)?.capabilities.listModels !== undefined
  })

  const providerModels = computed(() => {
    if (!getMetadataForProvider(activeSpeechProvider.value))
      return []
  ...
  async function loadVoicesForProvider(provider: string, model?: string) {
    ...
    const metadata = getMetadataForProvider(provider)
    if (!metadata)
      return []
    ...
    try {
      const voices = await metadata.capabilities.listVoices?.(providersStore.getProviderConfig(provider), model) || []
  ...
  function ensureActiveSpeechModel() {
    ...
    if (activeSpeechProvider.value !== OFFICIAL_SPEECH_PROVIDER_ID)
      return ensureDefaultListedSpeechModel()
  ...
  function ensureDefaultListedSpeechModel() {
  ...
  function ensureDefaultListedSpeechVoice(provider: string) {
  ...
  watch(activeSpeechProvider, async (newProvider) => {
    if (newProvider && !getMetadataForProvider(newProvider)) {
    ...
    ensureDefaultListedSpeechVoice(newProvider)
  ...
  else if (!getMetadataForProvider(activeSpeechProvider.value)) {
    activeSpeechProvider.value = 'speech-noop'
    activeSpeechModel.value = ''
    clearVoiceSelection()
  }
  ...
  onMounted(() => {
    ...
    loadVoicesForProvider(activeSpeechProvider.value, activeSpeechModel.value || undefined).then(() => {
      ...
      ensureDefaultListedSpeechVoice(activeSpeechProvider.value)
    })
  })
