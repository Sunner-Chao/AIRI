...
export const useHearingStore = defineStore('hearing-store', () => {
  ...
  const supportsModelListing = computed(() => {
    return getMetadataForProvider(activeTranscriptionProvider.value)?.capabilities.listModels !== undefined
  })

  const providerModels = computed(() => {
    if (!getMetadataForProvider(activeTranscriptionProvider.value))
      return []
  ...
  async function loadModelsForProvider(provider: string) {
    if (getMetadataForProvider(provider)?.capabilities.listModels !== undefined) {
