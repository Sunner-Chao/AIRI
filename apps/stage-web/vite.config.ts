...
const anthropicMessagesProxyPath = '/__airi/anthropic-messages-proxy'
...
function localAnthropicMessagesProxy(): PluginOption {
  return {
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use(anthropicMessagesProxyPath, async (req, res) => {
        if (!isLoopbackAddress(req.socket.remoteAddress)) {
          res.statusCode = 403
          res.end('Local Anthropic Messages proxy only accepts loopback requests')
          return
        }
        ...
        const target = req.headers['x-airi-anthropic-messages-url']
        if (typeof target !== 'string') {
          res.statusCode = 400
          res.end('Missing x-airi-anthropic-messages-url')
          return
        }
        ...
        try {
          ...
          res.statusCode = 400
          res.end('Invalid Anthropic Messages URL')
          return
        }
        ...
        for (const [key, value] of Object.entries(req.headers)) {
          if (
            ...
            || key === 'connection'
            || key === 'content-length'
            || key === 'x-airi-anthropic-messages-url'
          ) {
            continue
          }
        ...
        try {
          ...
          res.statusCode = 502
          res.end(errorMessageFrom(error))
        }
      })
    },
    name: 'proj-airi:local-anthropic-messages-proxy',
  }
}
